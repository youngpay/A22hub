(function (app) {
    ...
    self.showLoading = function(title) {
        if (title != undefined) {
            $('#loadingpaneltitle').text(title);
        }
        $('#loadingpanel').show();
    };
    self.hideLoading = function() {
        $('#loadingpanel').fadeOut();
        $('#loadingpaneltitle').text('正在载入');
    };

    self.bytesFormat = function(len) {
        ...
        return len.toFixed(1) + ' ' + unit[index];
    };
})(window.app = window.app || {});

$(document.body).on('dragover', function myfunction() {
    if (event.preventDefault) {
        event.preventDefault();
    }
    event.dataTransfer.dropEffect = "none";
}).ready(function () {
    $(document.body).height($(window).height() - 40);
});